...
for(year in 1:nYears){
  ...
  if(nDams==0){
    ...
    if(nrow(Adults)>0){
      ...
      Passage=exp((sBetas[,1]+sBetas[,2]*(Adults[,2]-65.711)/9.945)+sBetas[,3]*runif(nrow(Adults),0,30))
      Passage=ifelse(
        Passage>1,
        1,
        Passage
  ...
  if(exists("Spawners")){
    newRow[5]=nrow(subset(Spawners,Spawners[,1]==0))/nrow(subset(Spawners,Spawners[,1]==1))
  } else {
    newRow[5]=NA
  }
  newRow[6]=year
  if(exists("Spawners")){
    ...
    newRow[19]=sum(
      round(
        exp(3.07*log(subset(Spawners,Spawners[,1]==1&Spawners[,3]==3)[,2])-4.46),
        0
      ),
      na.rm=TRUE
    )
    newRow[20]=sum(
      round(
        exp(3.07*log(subset(Spawners,Spawners[,1]==1&Spawners[,3]==4)[,2])-4.46),
        0
      ),
      na.rm=TRUE
    )
    newRow[21]=sum(
      round(
        exp(3.07*log(subset(Spawners,Spawners[,1]==1&Spawners[,3]>=5)[,2])-4.46),
        0
      ),
      na.rm=TRUE
    )
